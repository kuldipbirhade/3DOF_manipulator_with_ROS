...
#include "myroboticarm_msgs/action/myroboticarm_task.hpp"
...
public:
    ...
    : Node("task_server", options)
  {
    ...
    action_server_ = rclcpp_action::create_server<myroboticarm_msgs::action::MyroboticarmTask>(
...
private:
  rclcpp_action::Server<myroboticarm_msgs::action::MyroboticarmTask>::SharedPtr action_server_;
  ...
  rclcpp_action::GoalResponse goalCallback(
      ...
      std::sharedsharedexplicitexplicit TaskServer(const rclcpp::NodeOptions& options = rclcpp::NodeOptions())
    : Node("task_server", options)_ptr<const myroboticarm_msgs::action::MyroboticarmTask::Goal> goal)
  {
  ...
  rclcpp_action::CancelResponse cancelCallback(
      const std::shared_ptr<rclcpp_action::ServerGoalHandle<myroboticarm_msgs::action::MyroboticarmTask>> goal_handle)
  {
    (void)goal_handle;
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<myroboticarm_msgs::action::MyroboticarmTask>> goal_handle)
  {
    RCLCPP_INFO(get_logger(), "Executing goal");
    auto result = std::make_shared<myroboticarm_msgs::action::MyroboticarmTask::Result>();
...
RCLCPP_COMPONENTS_REGISTER_NODE(myroboticarm_remote::TaskServer)